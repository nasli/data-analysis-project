...
```{r}
model <- lm(room_price ~ neighbourhood + room_type + availibility+0, data = train_data)
...
predict_room_price <- function(neighbourhood, room_type, availibility, coefficients) {
  # Intercepto
  ...
  predicted_price <- intercept + neighbourhood_coef + room_type_coef + availibility_coef * availibility + interaction_coef
...
```{r}
new_data <- data.frame(neighbourhood = "Sol", room_type = "Entire home/apt", availibility = 120)
...
1.  
    ...
    # Leer el archivo CSV y especificar los tipos de datos de las columnas
    ...
    library(dplyr)
    library(readr)
    df <- read.csv("barrios_municipio_madrid.csv", sep = ";", colClasses = c("character", "character"))
    ```

    ```{r}
    merged_df <- left_join(data, df, by = c("neighbourhood" = "barrio_nombre"))
    head(merged_df)
    merged_df <- na.omit(merged_df)
    set.seed(123) # Este set.seed hace que a todos nos generen los mismos número aleatorios
    idx <- sample(1:nrow(merged_df), 0.7*nrow(merged_df))
    df_train <- merged_df[idx,]
    df_test <- merged_df[-idx,]
    ```

    # Convertir las variables categóricas a factores

    ```{r}
    df_train$distrito_nombre <- as.factor(df_train$distrito_nombre)
    df_train$room_type <- as.factor(df_train$room_type)
    ...
    ```{r}
    rf_model <- randomForest(room_price ~ distrito_nombre + room_type, data = df_train, ntree = 500)
    ```
